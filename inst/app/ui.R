...
ui <- page_fluid(
 ...
 #-------------------------------------------------
  ...
  "),
  tags$style("input[type='text']{font-size: 12px;}"),
  tags$style("input[type='number']{font-size: 12px;}"),
  ...
  #------------------------------------------------
    ...
    card_header(class = "bg-primary text-white",
                h5("5x3: A Visualization Tool to explore tests and sample sizes before a study, & help interpret findings after data collection.")),
    shinyjs::useShinyjs(),
    actionButton("Manual", "Help", class = "btn-small",width=100), #value = FALSE, size = "small"),
    card_body(
      layout_sidebar(
        sidebar = sidebar(width = 1200, open = "open",
     h6("Given an interval [L, U] of effect sizes that are not materially significant (equivalent to zero), the tool considers 5 tests: whether the effect size E is
      ...
      h6("Select the Design or Analysis tab depending on your study's phase. Enter desired tests, study design parameters and real or anticipated data in the sidebar, which can be hidden.
      ...
      h6("Draw a box in the chart and double click in it to zoom. Double click again to unzoom. Hover on a chart position to get the co-ordinates and the strongest test with desired power (design phase) or strongest rejected test hypothesis (analysis phase)."),
    )),
  ...
  layout_sidebar(
    ...
    #-------------------------------------------------
    # Sidbar with controls
    #-------------------------------------------------

      # === Slider to specify equivalence region [L, U] === #

      h6("Slide to set interval [L, U] of values that are not materially significant."),
      uiOutput("Slider"),
      hr(),

      #== Panels to Nominate Tests (and desired power for design phase) ===#

      h6("Toggle to view or set tests."),
      layout_columns(switchInput("Alpha", "Tests", value = FALSE, size = "small"),
                      conditionalPanel("input.tab == 'Design'",
                          switchInput("Power", "Power", value = FALSE, size = "small"),
                       )),


      # === Study Description === #
      card(
        card_header("Enter study  description here."),
        layout_sidebar(
          sidebar = sidebar(width = 400, open = "closed",
         ...
         navset_tab(
          ...
          # === Tab 1: Design phase ===
           nav_panel(value = "Design",
                     ...
                     div(style = "width:350px;color:blue;font-size: 8px;",
                         ...
                         plotOutput("FigSS", height=500, width=500,
                          ...
                          #=====  Draggable legend =========
                         absolutePanel(plotOutput("LegendSS"), draggable = TRUE, height=300,width=300,top = 220, right=0),
           ...
           nav_panel( value = "Analysis",
                      ...
                      # =========Confidence intervals are displayed if a sidebar is opened======
                      card(
                        full_screen = TRUE,
                        card_header(class = "d-flex justify-content-between align-items-center",
                                    "Confidence/Compatability Intervals for each alpha."),
                        layout_sidebar(
                          sidebar = sidebar(width = 420, open = "closed",
                                            jqui_draggable(
                                              plotOutput("CIs"),
    ...
    card(style = "width: 800px;",
      layout_sidebar(
        sidebar = sidebar(width = 800, open = "open",layout_columns(
          switchInput("BW", "Monochrome chart.", value=F, size="mini"),
          switchInput("chartColor", "Change chart colors.", value = FALSE, size = "mini"), # a panel will open to switch to B&W display, or to browse for file containing color palette
          switchInput("newTerminology", "Use informal terminology.", value = FALSE, size = "mini"),
          switchInput("newRange", "Change chart ranges.", value = FALSE, size = "mini"),
        ),
       conditionalPanel("input.newRange",
        layout_sidebar(
          sidebar = sidebar(
                            p("Effect size"),
                            layout_columns(
     ...
     ))) # end of card with display option
  ))
#========= end of UI ============#
